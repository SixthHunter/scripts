...
ipcab2()
{
	...
	while read line
	do 	val="$( jq -r ".[]|select( .p == \"${line}\")|.v" <<< "$data" )"
		tval="$( bc <<< "scale=16; (1+ ( $val /100) ) * ${tval:-1}" )"
		tvalp="$( bc <<< "scale=16; ( $tval -1) * 100" )"
...
{
	if [[ ${*} =~ (-h|:?help) ]]
	then curl -s 'rate.sx/:help' ;return
	fi
