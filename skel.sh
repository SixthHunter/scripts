...
# v0.2.12  oct/2021  by mountaineerbr
...
#required packages
if ! command -v pkgA pkgB pkgC &>/dev/null
then echo "$SN: err  -- pkgA, pkgB and pkgC are required" >&2 ;exit 1
fi
...
#bash semaphore
#while jobs=( $(jobs -p) ) ;((${#jobs[@]} > JOBMAX)) ;do sleep .2 ;done

#zsh semaphore
#while ((${#jobstates[@]} > JOBMAX)) ;do sleep .2 ;done
